#!/usr/bin/env python3
"""Build the self-contained S02 page: one HTML file with the snapshot, the standard
renderer, the engine, the records-cortex view, the world, the stores, the planner and the
page script inlined, so it runs from file:// and on GitHub Pages.
...
def main() -> int:
    ...
    if snapshot.get("extra", {}).get("stage") != "S02":
        raise SystemExit("the snapshot is not an S02 fixture (tools/parity_fixture.py writes extra.stage)")
    modules = [
