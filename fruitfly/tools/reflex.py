...
def main() -> None:
    ...
    ap.add_argument("--out", default=str(ROOT / "receipts" / "g2_reflex_facts.json"))
    args = ap.parse_args()
    fly = load_fly()
    protocol = reflex_protocol()
    ...
    receipt = Receipt.build("cadence-fruitfly.g2-reflex-facts/1", body, sources=[
        ("fruitfly/banc.py", ROOT / "fruitfly" / "banc.py"), ("fruitfly/brain.py", ROOT / "fruitfly" / "brain.py"),
        ("fruitfly/protocol.py", ROOT / "fruitfly" / "protocol.py"), ("tools/reflex.py", Path(__file__)),
