...
``prepare <stage> <release>`` adds the sha256 of every checkpoint file and the release tag
to ``<stage>/checkpoints.json`` and prints the upload command; the files themselves stay
out of the repository. ``verify <stage> <directory>`` checks downloaded files against the
manifest, which the Pages workflow runs after ``gh release download``."""
import hashlib
import json
import sys
