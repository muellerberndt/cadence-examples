...
Every owner holds a potential, publishes a bounded activation and reads incoming
messages. Local updates repair the difference between its potential and its
inbox plus external drive and bias. The named regions below belong to one graph.
A phrase is emitted only after comparing isolated candidate trajectories.

| Component | Owners | Role | Where its information comes from |
| --- | ---: | --- | --- |
| Auditory history and brief | 636 | Four prior note events, pitch-class history, beat/phrase position, mode, style and arousal | Supplied event encoder; learned outgoing seams |
...
| Motif recall | 61 | Recalls the learned pitch at a cued motif position | 366 fast delta-rule seams, plus reciprocal links to note intention |
| Expectation cue | 192 | Previous chord by mode; previous duration by arousal and beat | Supplied context readback |
| Harmonic and rhythmic expectation | 36 | Expected next chord (24) and onset duration (12) | 2,880 seams from smoothed corpus transition tables, bridged both ways to note intention |

The pretrained sizes H=96, 256 and 768 contain 975, 1,295 and 2,319 owners.
The performing brain adds 67 motif owners and 488 directed seams: 366 memory
seams and 61 links in each direction between recall and note intention. It adds
228 expectation owners with 2,880 expectation seams and 36 links in each direction
between expectation and note intention. Counts,
checkpoint hashes and per-region lesions are recorded with every assessment.
The additional motif seams learn only after a real phrase is selected; an
...
All arrows are actual seams of one Cadence settlement. Reciprocal traffic lets
note intention and its contexts affect one another. `MotifBrain` uses the public
`couple` pattern and `FastSeams` delta updates; it introduces no new owner rule.
...
owners. A supplied explicit critic compares scale fit, strong-beat harmony,
...
The first six committed melody pitches write the motif seams. Later cue inputs
activate their recall population, which influences note intention through actual
bridges in the joint graph. A running list also retains the score for editing;
...
it contained. Exact notes never enter this memory, and the trained event seams stay
...
continues the same owner rule in chunks, with more early samples for a displayed
rehearsal, until the maximum owner-equation
...
The viewer measures every owner for signed population means and RMS mismatch.
All 2,614 performing owners and 1,702,938 directed seams are mapped, using a
binary full-graph export and WebGL2. No owners or seams are dropped to create a
representative diagram. Zoom reveals dense overlaps. Region colors identify
wiring roles; separate channels show activation, equation error, state change
and actual seam plasticity. Every fourth note iteration samples the actual
...
external drive in an isolated copy, retaining learned biases and seams. Some
...
- Public `couple` is enough to graft a new memory circuit onto a trained graph.
- `FastSeams` supplies one-trial content storage; addressing remains a design choice.
...
Four GPU replicas settle different training rows. Raw free/nudged seam contrasts
