...
const deaths = []; pop.onDeath = b => deaths.push({ age: b.age, cells: G(b.g, 'cells'), horizon: G(b.g, 'horizon'), slow: G(b.g, 'slowRate'), noise: G(b.g, 'noise'), sleep: b.sleeper ? 1 : 0, dreams: b.dreams, slept: b.sleptTicks, nodes: b.nGenome, killed: !!(b.pain > 0), errEarly: b.nEarly ? b.errEarly / b.nEarly : null, errLate: b.nLate ? b.errLate / b.nLate : null, plans: b.plans, accepted: b.planAccepted, confirmed: b.planConfirmed, eats: b.eats, bites: b.bites, dist: b.dist });
...
for (let t = 1; t <= ticks; t++) {
  ...
  if (t % every === 0 || t === ticks) {
    ...
    for (const k of ['cells', 'horizon', 'slowRate', 'noise', 'splitAt', 'freq', 'dFood', 'dEnergy', 'dPain', 'dSpeed', 'planRadius', 'sleep']) { const c = {}; for (const b of bs) { const v = G(b.g, k); c[v] = (c[v] || 0) + 1; } census[k] = c; }
    ...
    const sleepers = bs.filter(b => b.sleeper).length, asleep = bs.filter(b => b.sleeping).length;
    ...
    const line = { kind: 'tick', t, n, mass: pop.mass - m0, births: pop.births, deaths: pop.deaths, kills: pop.kills, seconds: (Date.now() - t0) / 1000,
      alive: { records: withRecords, plan: withPlan, slow: withSlow, sleepers, asleep, nodesMean: mean(bs.map(b => b.n)), genomeNodesMean: mean(bs.map(b => b.nGenome)), musclesMean: mean(bs.map(b => b.g.springs.filter(s => s.muscle).length)), speedMean: mean(bs.map(b => Math.abs(b.speed))), ageMean: mean(bs.map(b => b.age)), energyMean: mean(bs.map(b => b.energy)), lineages: new Set(bs.map(b => b.lineage)).size },
      lifetime: { all: lifeBy('all', () => true), sleep: lifeBy('sleep', d => d.sleep === 1), noSleep: lifeBy('noSleep', d => d.sleep === 0 && d.slow > 0), records: lifeBy('records', d => d.cells > 0), noRecords: lifeBy('noRecords', d => d.cells === 0), plan: lifeBy('plan', d => d.horizon > 0), noPlan: lifeBy('noPlan', d => d.horizon === 0), slow: lifeBy('slow', d => d.slow > 0), noSlow: lifeBy('noSlow', d => d.slow === 0), killed: lifeBy('killed', d => d.killed) },
      model: { errEarly: mean(deaths.filter(d => d.errEarly !== null).map(d => d.errEarly)), errLate: mean(deaths.filter(d => d.errLate !== null).map(d => d.errLate)), errLateSleep: mean(deaths.filter(d => d.sleep === 1 && d.errLate !== null).map(d => d.errLate)), errLateNoSleep: mean(deaths.filter(d => d.sleep === 0 && d.slow > 0 && d.errLate !== null).map(d => d.errLate)), dreamsMean: mean(deaths.filter(d => d.sleep === 1).map(d => d.dreams)), sleptMean: mean(deaths.filter(d => d.sleep === 1).map(d => d.slept)), errLateRecords: mean(deaths.filter(d => d.errLate !== null && d.cells > 0).map(d => d.errLate)), errLateNoRecords: mean(deaths.filter(d => d.errLate !== null && d.cells === 0).map(d => d.errLate)) },
