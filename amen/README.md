...
The page carries the brains trained on the full corpus, one per Cadence release; each has a receipt under `runs/record-composer-v12/` (0.11.0). A receipt names the composer run's own sealed receipt
...
The page opens with Three corpora, on 0.11.0 (`default` in `web/models/index.json`). Earlier
