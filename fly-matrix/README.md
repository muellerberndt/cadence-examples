...
- **Cadence version:** 0.15.0, the release that carries what this example sent back into the library (the efficacy cap as configuration, the critic on the raw error under centring, the saturation and trace readings of the learn report, the viewer's anatomical mode with its palette, label and pixel-ratio options). The receipts name the version their numbers were produced with.
